...
const CostSheet = () => {
    ...
    const fetchDetails = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:5000/api/costsheet/getcostsheetdata/${event}`
        );
        console.log(data);
        if (data.costsheetData && data.eventDetails) {
          ...
          const res = await axios.get(
            `http://localhost:5000/api/event/getEventCodes/${data.eventDetails[0].projectHead}`
          );
  ...
  const handleEventDetails = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `http://localhost:5000/api/event/getEventCodes/${e.target.value}`
      );
      console.log(data);
  ...
  const handleCostSheetSubmission = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `http://localhost:5000/api/costsheet/addcostsheet`,
        {
  ...
  const handleUpdateBtn = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "http://localhost:5000/api/costsheet/updatecostsheet",
        {
